...
//
// # Say which proxies you trust
//
// The first three sources below are request headers, which a client controls
// completely. Pass WithTrustedProxies with the networks your load balancer,
// ingress or CDN egress actually comes from, and RealIP only believes those
// headers on requests arriving from them:
//
//	router.Use(middleware.RealIP(middleware.WithTrustedProxies("10.0.0.0/8")))
//
// A request from anywhere else is keyed on RemoteAddr, which cannot be forged
// over TCP.
//
// With no options, RealIP believes the headers on every request. That is
// correct behind a proxy that overwrites them, and unsafe on a server
// reachable directly from the internet: there, a caller sends an arbitrary
// X-Forwarded-For per request and defeats anything keyed on the result.
// RateLimit is keyed on it by default, so the practical consequence is that
// the rate limit stops working - each forged header looks like a new client.
// WithTrustAnyProxy makes that choice explicit when it is deliberate.
//
// # Source order
//
...
func RealIP(options ...RealIPOption) routing.Middleware {
	var config realIPConfig
	...
	return func(
		...
		return routing.Wrap(next, http.HandlerFunc(func(
			...
			// Headers are only consulted when the request came from a proxy
			// this middleware was told to trust; otherwise they are attacker
			// input and RemoteAddr is the only honest answer.
			if config.trusts(request.RemoteAddr) {
