...
* **`cmd/`+`internal/` pattern.** Each runnable example is `examples/cmd/<name>`:
  ...
  `GET`+`PUT` via `httpx/patch.From`). Shared code — logger, custom validator
  registration, the sample handler — lives in `examples/internal/*`, not
  importable from outside `examples/` per Go's own rule. That rule is also why
  `examples` lists **itself** in its own `mayDependOn` in `.go-arch-lint.yml`:
  without it, the `cmd/* -> internal/*` cross-import is blocked even though
  both sides are the same component. New example: create `examples/cmd/<name>`,
  reuse what's in `examples/internal`, duplicate only what's specific to it.
