...
// SpecVersion is the version of the OpenAPI Specification a Document declares.
// Not called "Version": Info.Version already means the API's own version, a
// different concept.
type SpecVersion string

// Specification versions this package can emit.
//
// Only 3.1 and 3.2 are offered, and that is a correctness constraint rather
// than a shortlist: the generated schemas express nullability as a type array
// (`"type": ["string", "null"]`), which is how 3.1 replaced the 3.0 "nullable"
// keyword. Declaring 3.0.x while emitting that would describe the document
// wrongly, so it is not an option.
//
// The difference between the two below is the version string alone. Nothing
// this package generates uses a 3.2-only construct, so the same document is
// valid under either - which is what makes SpecVersion31 a usable escape hatch
// when a consumer's tooling has not caught up to 3.2 yet.
const (
	// SpecVersion31 is OpenAPI 3.1.1.
	SpecVersion31 SpecVersion = "3.1.1"

	// SpecVersion32 is OpenAPI 3.2.0, and the default.
	SpecVersion32 SpecVersion = "3.2.0"
)
...
type Document struct {
	...
	OpenAPI SpecVersion `json:"openapi"`
	...
	// Not represented yet: JSONSchemaDialect (3.1+), Webhooks (3.1+,
	// top-level webhook registration) and Security. Security is the one
	// with a real prerequisite - it needs Components.SecuritySchemes,
	// which in turn needs the authentication support this framework
	// does not have.
